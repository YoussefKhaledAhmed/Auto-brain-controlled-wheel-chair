...
sys.path.insert(1 , ".//Recording")
sys.path.insert(1 , ".//Recording//venv//Py3")
class Collect:
    ...
    def record(self, no_of_seconds):  # Secs number
        ...
        @return DataFrame
        """
        no_of_seconds = int(no_of_seconds)
        if self.emotiv_hs == True:  # Emotive
            ...
            for i in range(no_of_seconds * 128):
                ...
                # print("\n new\n")
                # print(type((self.record_eeg())[0]) , "  length-> ",len(self.record_eeg()))
                # print("\n")
...
import csv
import os
def save(user_name,Data,label = -1):
    'data in dataframe'
    # file = os.path.join(os.getcwd(), ) + user_name + '.csv'
    if label!= -1: Data['Label'] = label
    pd.DataFrame(Data).to_csv(user_name,header=False if os.path.isfile(user_name) else True, index=False,mode='a')
...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        # print(Data[0])
        save(user_name='Youssef',Data=Data,label=i)
       # print('break!')
        #time.sleep(4)
        #print('ready !')
        #time.sleep(1)
